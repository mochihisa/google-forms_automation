...
def main():
    ...
    for s in random.sample(list(range(2101, 2141)), k=random.choice(list(range(35, 41)))) + \
        ...
        date = f'2020/10/16 {np.random.choice([int(i) for i in np.random.choice([str(list(range(5, 8))), str([8])], p=[0.95, 0.05]).strip("[").strip("]").split(", ")])}:{random.randint(00,59)}:{random.randint(00,59)} 午前 GMT+9'
        temperature = random.choice([float(f) for f in np.random.choice([str([round(i, 1) for i in np.arange(36, 37, 0.1)]), str(
            [round(i, 1) for i in np.arange(37, 39, 0.1)])], p=[0.95, 0.05]).strip('[').strip(']').split(', ')])
        condition = np.random.choice(['あり', 'なし'], p=[0.05, 0.95])
        res = res + [[date, s, temperature, condition, None]]
        print(res)
    file_write(res)
